...
- 按钮为紧凑详情标题栏左侧的图标按钮，视觉、尺寸、hover、pressed、focus、disabled 与 Tooltip 均引用统一基线。
...
- 剧详情使用与电影详情相同的 2:3 海报和右侧 Hero 对齐关系；Season 列表行为分隔线列表，不使用逐行外边框卡片。
- Season 行从上到下固定展示：特别篇 / 第 x 季、季名、首播日期、播放源集数、已看集数；已看为 0 时使用 `未看`。
- 季详情保留 2:3 海报完整高度，右侧 Hero 缩短，下面预留按钮带。
- 季详情右侧评分使用一张短横向 TMDB 卡片，分成星标 / 来源、评分 / 星级、票数三列。
- 季详情按钮带固定为偏好、已看状态、不想看、修正季归属；不提供默认源播放按钮。
- Episode 行使用分隔线列表，固定展示集号、集名、两行简介、首播日期、播放源数量、`hh:mm:ss` 时长、最近播放，以及详情 / 默认源播放动作。
...
## 12. Current Detail Metadata Addendum

当前剧、季、集详情使用所属 `TvSeries` 的同一组持久化 metadata，不在 Season 或 Episode 重复存储：
...
剧详情额外展示制作状态、总季数和总集数。季详情额外展示总集数和已看集数。缺失字段显示 `-` 或当前统一空态，不伪造数据。

媒体库剧列表和季列表同样复用所属 `TvSeries` 的导演和主演字段。导演与主演区域只在内容溢出时显示完整文本 Tooltip。
