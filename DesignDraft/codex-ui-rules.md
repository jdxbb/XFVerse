...
- 观影洞察统计卡为已看、喜爱、想看、不想看四项；统计支持全部 / 本月，趋势统一写为“较上月”。
- 首页片库预览使用全部统计，趋势统一写为“较上月”。
...
- 按钮固定在详情路由的紧凑标题栏左侧，不与壳层侧边栏展开 / 收起控件争夺位置；左右内边距与右侧关闭按钮保持对称。
...
## 11.1 Shared Liquid Glass Baseline

首页、媒体库和现有详情页已经接入统一的液态玻璃视觉资源。Phase 7.8 及后续页面重构必须继续复用这套资源，不允许在单个页面内各自硬编码一套玻璃样式。
...
- Popup、Dialog、菜单、标签及后续新增的同类背景承载控件。

实现约束：

- 使用公共 ResourceDictionary 和 Light / Dark 对应资源；
- 保持克制、高级、可读，使用静态弥散背景、轻透明、柔和高光、低对比边框和有限阴影；
- Light / Dark 都要有轻微模糊观感，但禁止实时背景模糊；通过缓存后的背景、乳白或深色半透明层和边缘高光实现；
- 叠加卡片必须用不同透明度、边框高光和阴影深度区分层级，避免上层卡片融入下层；
- 缺少真实海报的详情页使用统一占位海报参与静态背景配色；
- 静态详情背景需要保留页面亮度，不允许用过深的单一底色吞掉海报衍生色；Light / Dark 都要保留多色弥散层和明确的内容层级；
- 海报来源切换时，静态背景调色和发光结果必须带请求版本校验；旧海报的异步结果不得覆盖后续占位图或新海报；
- 海报为空时必须立即回退到统一占位海报调色和发光，不得沿用上一条详情的缓存视觉；
- 不做高饱和炫光、过量透明、复杂动态纹理或影响扫描阅读的装饰；
- hover、pressed、selected、disabled、loading 状态必须清晰可区分；
- 当前详情页静态海报衍生背景不是公共卡片样式的替代品；
- 首页、媒体库和现有详情页已接入公共卡片、带背景按钮和主要选中态样式；
- 其它页面在对应重构阶段接入，Phase 7.8 负责跨页一致性、Light / Dark 可读性和性能回归。
