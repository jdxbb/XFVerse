...
- 顶层 Tab 改为标题栏下方横线上的左对齐样式：无边框，当前 Tab 使用强调色文字和同色下划线，内容区从横线下方开始；搜索 Tab 使用短文案 `影片`，避免和 Shell 页面标题重复。
...
- Movie / TV 的地区、语言、收藏状态改为媒体库式多选：`全部` 清空并关闭菜单，非全部项可多选且保持菜单打开，按钮文案最多展示两个选项，超过两个显示数量。
- `清除筛选` 只重置筛选选项和分页，不清空搜索词，也不清空当前搜索结果池。
- Movie 搜索筛选拆分为：观看状态 `全部 / 已看 / 未看`、播放源 `全部 / 有播放源 / 无播放源`、收藏状态 `全部 / 喜爱 / 想看 / 不想看`。
- TV 搜索筛选拆分为：播放源、观看状态、收藏状态；TV 收藏状态同样使用多选逻辑，TV 观看状态通过 Series 下 Episode 已看状态聚合投影。
- 搜索方式按钮文案改为 `搜索方式：电影 / 人物` 或 `搜索方式：电视剧 / 人物`；媒介按钮文案改为 `影视：电影 / 电视剧`。
- 搜索结果区和榜单内容区增加单次运行内滚动进度持久化：切换导航页或进入详情页再返回时恢复原 offset；新搜索、筛选重建、翻页或重载榜单会回到顶部；重启软件后不保留。

验收标准：

- 页内不再出现重复 `影片搜索` 标题，也不再有包住整个页面的大边框背景。
- Tab 首项左边缘与标题栏标题左边缘保持同一内容列；Tab 字号小于标题栏标题；Tab 文案应避免重复 Shell 标题。
...
- Movie / TV 地区、语言、收藏状态支持多选并保持菜单打开；选择 `全部` 清空并关闭菜单。
- `清除筛选` 不清搜索词、不清结果池。
...
- 多选筛选统一规则：`全部` 表示空集合；非全部项 `StaysOpenOnClick=True`；按钮文案按选项顺序显示，0 项为 `全部`，1-2 项直接拼接，超过 2 项显示数量。
