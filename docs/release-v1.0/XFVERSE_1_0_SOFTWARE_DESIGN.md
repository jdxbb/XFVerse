...
- 产品版本：XFVerse 1.0.0。
- 应用类型：Windows 桌面应用。
- 正式候选平台：Windows 10 / Windows 11 x64，以及 Windows ARM64；最终支持范围由 Phase 8.9 RC 证据收口。
...
- 不重新设计或修改实际 UI。
- 不描述尚未进入当前代码的设想功能。
- 不把 X86、自动更新、Portable 或云账号写成 1.0 能力。
...
两个安装包使用相同正式 AppId、产品版本和用户数据格式。切换架构时覆盖程序文件并清理另一架构的 mpv 与 ffprobe 程序目录，用户数据目录不参与安装器清理。

### 11.2 原生资源

应用发布目录需要包含：

- 与目标 RID 匹配的 x64 或 ARM64 `libmpv-2.dll`。
- 与目标 RID 匹配的 ffprobe。
- WPF 资源。
...
| 正式平台 | Windows x64 / Windows ARM64 独立 self-contained 安装包。 |
| 更新 | 1.0 手动下载安装器升级。 |

## 15. 1.0 限制与非目标

- X86 不属于正式支持。
- x64 与 ARM64 的干净环境应用级 RC 尚未完成，最终系统版本支持声明不得早于 Phase 8.9。
- 没有自动更新器。
- 没有 Portable 包。
- 没有云账号和跨设备同步。
- “退出登录”未接入。
